...
impl MathematicaServer {
  ...
  async fn time(
    &self
  ) -> Result<Json<TimeResult>, String>
  {
    let now_local = Local::now();
    Ok(Json(TimeResult {
      local_rfc3339: now_local
        .to_rfc3339(),
...
impl ServerHandler
  for MathematicaServer
{
  fn get_info(&self) -> ServerInfo {
    ServerInfo {
      // You can add richer text here if
      // you want the client to
      // understand semantics.
      instructions: Some(
        "Mathematica/Wolfram MCP \
         server. Use \
...
#[derive(
  Debug, Serialize, schemars::JsonSchema,
)]
