...
enum Command {
  /// Run the MCP server over stdio (for
  /// Continue/Claude Desktop/etc.)
  Serve,
  /// Interactive REPL that calls the
  /// same tools locally (no MCP host
  /// needed)
  Repl
}

fn init_tracing() {
  // IMPORTANT: server mode must not
  // write to stdout (stdio transport
