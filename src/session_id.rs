//! src/session_id.rs
//!
//! A small, **readable** replacement for Python's `animalid.AnimalIdGenerator`.
//!
//! Goal:
//! - Generate human-friendly session IDs like `quick_fox-kind_sloth-bright_auk-calm_mole`
//! - Allow `verify(session_id)` to detect tampering (no extra signature suffix in the ID)
//!
...
//! - The checksum is derived from `HMAC-SHA256(secret_key, payload32)` (top 12 bits).
//! - On verify: decode words -> recover payload32 + checksum12 -> recompute checksum12 -> compare.
//!
//! This mirrors the intent of Python `AnimalIdGenerator.generate()` and `.verify()`
...
/// This is intentionally named to match the Python library semantics.
///
/// The rest of the Rust code can keep calling:
...
impl AnimalIdGenerator {
    /// Load the secret key from `ANIMALID_SECRET_KEY` or fall back to a dev default.
    pub fn from_env() -> Self {
        let secret =
            std::env::var(ENV_SECRET_KEY).unwrap_or_else(|_| DEFAULT_SECRET_KEY.to_string());
    ...
    /// Generate a new tamper-evident session id.
    pub fn generate(&self) -> String {
        // 32-bit random payload
        let payload = random_u32();

        // 12-bit checksum (HMAC over payload)
        let checksum12 = self.checksum12(payload);

        // Pack into 44 bits: [payload32 | checksum12]
        let packed: u64 = ((payload as u64) << 12) | (checksum12 as u64);

        // Split into 4 chunks of 11 bits (most-significant chunk first)
    ...
    /// Verify that a session id is well-formed and untampered.
    pub fn verify(&self, session_id: &str) -> bool {
        ...
        // Decode 4 x 11-bit indices back into packed 44-bit value
    ...
    fn checksum12(&self, payload: u32) -> u16 {
        let mut mac = HmacSha256::new_from_slice(&self.key).expect("HMAC accepts any key length");
        mac.update(&payload.to_be_bytes());
        let digest = mac.finalize().into_bytes();

        // Take the top 12 bits of the first 2 bytes as checksum
        // Example: bytes = ab cd -> checksum = (ab cd) >> 4
...
/// We need a 2048-entry "wordlist" (because each word encodes 11 bits).
///
/// Instead of shipping a gigantic 2048-word list, we generate 2048 unique tokens
/// by combining 64 adjectives × 32 animals = 2048.
///
/// Each encoded "word" looks like `quick_fox` (underscore is safe because the ID uses `-`
...
mod tests {
    ...
    fn tamper_one_char_fails() {
        ...
        // mutate the last char in the string (guaranteed to change something)
