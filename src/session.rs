...
impl SessionManager {
  ...
  pub async fn create_session(
    ...
    let join = thread::spawn(
      move || {
        // Create link *inside* the
        // thread.
        let mut kernel =
          match wolfram::launch_link(
            &kernel_cmd
          ) {
            | Ok(k) => {
              let _ =
        ...
        // When thread exits, Link
        // drops; kernel should
        // terminate.
        tracing::info!(
          "session thread exited"
        );
      }
    );
